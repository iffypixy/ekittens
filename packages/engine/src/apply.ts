...
export type Outcome = Result<{ state: MatchState; events: readonly DomainEvent[] }, GameError>;

const succeed = (state: MatchState, events: readonly DomainEvent[] = []): Outcome =>
  ok({ state, events });
const fail = (error: GameError): Outcome => err(error);

const isAlive = (state: MatchState, id: PlayerId): boolean => !state.out.includes(id);

const findPlayer = (state: MatchState, id: PlayerId): Player | undefined =>
  state.players.find((player) => player.id === id);

const cardInHand = (player: Player, card: CardId): Card | undefined =>
  player.hand.find((held) => held.id === card);

/** The next alive player after `from`, walking the fixed seat order. */
const nextTurn = (state: MatchState, from: PlayerId): PlayerId => {
...
const removeFromHand = (hand: readonly Card[], cards: readonly CardId[]): readonly Card[] => {
...
const endTurn = (
  state: MatchState,
): { turn: PlayerId; pendingTurns: number; events: readonly DomainEvent[] } => {
