...
import { RatingsRepository } from "./services/ratings/repository.ts";
import { RatingsService } from "./services/ratings/service.ts";
import { RelationshipsRepository } from "./services/relationships/repository.ts";
import { RelationshipsService } from "./services/relationships/service.ts";
import { UsersRepository } from "./services/users/repository.ts";
...
const main = async (): Promise<void> => {
  ...
  const sessions = new SessionStore(redis, config.SESSION_TTL_SECONDS);
  const users = new UsersService(new UsersRepository(db));
  const relationships = new RelationshipsService(new RelationshipsRepository(db));
  const ratings = new RatingsService(new RatingsRepository(db));
